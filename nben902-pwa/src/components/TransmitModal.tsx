...
type TransmitStatus = 'idle' | 'sending' | 'success' | 'error';
                    ...
                    {status !== 'idle' && (
                        <div className="space-y-2">
                            <div className={`rounded-lg p-3 text-sm flex items-start gap-3 ${status === 'error' ? 'bg-red-900/20 text-red-200 border border-red-900/50' :
                                status === 'success' ? 'bg-green-900/20 text-green-200 border border-green-900/50' :
                                    'bg-zinc-800 text-zinc-300'
                                }`}>
                                {status === 'sending' && <Loader2 size={18} className="animate-spin text-blue-400 shrink-0 mt-0.5" />}
                                {status === 'success' && <CheckCircle size={18} className="text-green-400 shrink-0 mt-0.5" />}
